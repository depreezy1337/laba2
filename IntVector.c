...
IntVector* int_vector_new(size_t initial_capacity) {
 ...
 int int_vector_set_item(IntVector* v, size_t index, int item) { //не может быть меньше нуля
	 if (index >= v->size) {
		 return -1;
	 }
 ...
 int int_vector_push_back(IntVector* v, int item) {
	 ...
	 if (v->size == v->capacity) { // если место закончилось 
		 v->capacity==0? (v->capacity=1):(v->capacity *= 2);//определение ёмкости нового вектора
